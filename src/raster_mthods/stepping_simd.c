#define GRAFIKA_TITLE ("grafika - stepping_simd")
#include "common.h"
...
struct stepping_simd raster_state = {0};
...
static void draw_triangle(const struct triangle t)
{
    ...
    __m128 x_starting_pixel = _mm_set1_ps((float)AABB[0]);
             ...
             E2 = _mm_add_ps(E2, B2_inc))
    {
        ...
        {
            __m128 edge_test_result = _mm_and_ps(_mm_and_ps(
                                                     _mm_cmplt_ps(alpha, mm_0),
                                                     _mm_cmplt_ps(betaa, mm_0)),
                                                 _mm_cmplt_ps(gamma, mm_0));

            if (_mm_movemask_ps(edge_test_result) == 0) continue;
            ...
            __m128i final_colour = _mm_shuffle_epi8(
                ...
                // unsigned char *texcolour0 = texdata + _mm_extract_epi32(tex_offset, 0);
                // unsigned char *texcolour1 = texdata + _mm_extract_epi32(tex_offset, 1);
                // unsigned char *texcolour2 = texdata + _mm_extract_epi32(tex_offset, 2);
                // unsigned char *texcolour3 = texdata + _mm_extract_epi32(tex_offset, 3);
            ...
            const __m128i write_mask    = _mm_castps_si128(write_mask);
            ...
            _mm_store_si128((__m128i *)pixel_location, masked_output);
#else
            _mm_maskstore_epi32((int *)&rend.pixels[pixel_index], _mm_castps_si128(write_mask), final_colour);
#endif
        }
    }
}
