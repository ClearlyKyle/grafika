...
struct binning_simd raster_state = {0};

static inline __m128 interpolate_values(const __m128 F[3], const __m128 V[3])
{
#if 1
    return _mm_add_ps(_mm_mul_ps(V[2], F[2]), _mm_add_ps(
                                                  _mm_mul_ps(V[1], F[1]),
                                                  _mm_mul_ps(V[0], F[0])));
#else
    // (V2 * F2) + (V1 * F1) + (V0 * V1);
    // _mm_fmadd_ps = (A * B) + C
    return _mm_fmadd_ps(V[2], F[2], _mm_fmadd_ps(V[1], F[1], _mm_mul_ps(V[0], F[0])));
#endif
}

static inline __m128 interpolate_values2(__m128 f0, __m128 f1, __m128 f2,
                                         __m128 v0, __m128 v1, __m128 v2)
{
    return _mm_add_ps(_mm_mul_ps(v2, f2), _mm_add_ps(
                                              _mm_mul_ps(v1, f1),
                                              _mm_mul_ps(v0, f0)));
}
...
static void _shade_full_tile(uint32_t pos_x, uint32_t pos_y, vec3 E0, vec3 E1, vec3 E2, vec2 t0, vec2 t1, vec2 t2, vec3 Z)
{
    ...
    __m128 mm_U0, mm_U1, mm_U2;
    mm_U0 = _mm_set1_ps(t0[0]);
    mm_U1 = _mm_set1_ps(t1[0]);
    mm_U2 = _mm_set1_ps(t2[0]);

    __m128 mm_V0, mm_V1, mm_V2;
    mm_V0 = _mm_set1_ps(t0[1]);
    mm_V1 = _mm_set1_ps(t1[1]);
    mm_V2 = _mm_set1_ps(t2[1]);

    __m128 mm_Z0, mm_Z1, mm_Z2;
    mm_Z0 = _mm_set1_ps(Z[0]);
    mm_Z1 = _mm_set1_ps(Z[1]);
    mm_Z2 = _mm_set1_ps(Z[2]);
                  ...
                  mm_edge_start_2 = _mm_add_ps(mm_edge_start_2, mm_step_y_2))
    {
        ...
        {
            TIMED_BLOCK_BEGIN(interp_depth);
            ...
            __m128 r = _mm_add_ps(f0, _mm_add_ps(f1, f2));
            r        = _mm_rcp_ps(r);
            ...
            const __m128 mm_pix_Z = _mm_add_ps(_mm_add_ps(_mm_mul_ps(mm_Z0, f0),
                                                          _mm_mul_ps(mm_Z1, f1)),
                                               mm_Z2);
            ...
            TIMED_BLOCK_END(interp_depth);

            if ((uint16_t)_mm_movemask_ps(mm_depth_test_result) == 0x0) continue;

            // write interpolated Z values
            _mm_maskmoveu_si128(_mm_castps_si128(mm_pix_Z),
                                _mm_castps_si128(mm_depth_test_result),
                                (char *)depth_location);

            // interpolate texture coordinates
            __m128 pixU = interpolate_values2(f0, f1, f2, mm_U0, mm_U1, mm_U2);
            __m128 pixV = interpolate_values2(f0, f1, f2, mm_V0, mm_V1, mm_V2);

            // clamp the vector to the range [0.0f, 1.0f]
            pixU = _mm_max_ps(_mm_min_ps(pixU, mm_1), _mm_setzero_ps());
            pixV = _mm_max_ps(_mm_min_ps(pixV, mm_1), _mm_setzero_ps());
            ...
            // NOTE : this method doesnt work as the texture RGB order does not match the screen RGB order
            // const int32_t *offsets = (const int32_t *)&tex_offsets;
            //__m128i final_colour = _mm_set_epi32(*(const int32_t *)(tex_data + offset[3]),
            //                                     *(const int32_t *)(tex_data + offset[2]),
            //                                     *(const int32_t *)(tex_data + offset[1]),
            //                                     *(const int32_t *)(tex_data + offset[0]));

            uint32_t *pixel_location = &rend.pixels[pixel_index];
#if 0
                const __m128i write_mask    = _mm_castps_si128(mm_depth_test_result);
...
void draw_object(struct arena *arena)
{
    ...
    for (uint32_t i = 0; i < tiles_queue_index; i++)
    {
        ...
        struct tile        *t_data   = &tile_array[tile_lookup_index];
        struct tile_coords *t_coords = &tile_coords[tile_lookup_index];

        uint32_t tile_x = t_coords->tile_x;
        uint32_t tile_y = t_coords->tile_y;

        uint32_t tile_triangle_count = t_data->triangle_count;
        for (uint32_t b = 0; b < tile_triangle_count; b++)
        {
